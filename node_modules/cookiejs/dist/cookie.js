      ...
      if (isPlainObject(name)) {
        ...
        var opt = isPlainObject(options) ? options : {
          expires: options
            ...
            path = opt.path !== undefined ? ";path=".concat(opt.path, ";path=/") : ';path=/',
            domain = opt.domain ? ";domain=".concat(opt.domain) : '',
            secure = opt.secure ? ';secure' : ''; //过期时间

        if (typeof expires === 'string' && expires !== '') {
          _readOnlyError("expires");
        } else if (typeof expires === 'number') {
          _readOnlyError("expires");
        }

        if (expires !== '' && 'toGMTString' in expires) {
          ";expires=".concat(expires.toGMTString()), _readOnlyError("expires");
        }

        var sameSite = opt.sameSite ? ";SameSite=".concat(opt.sameSite) : '';
        document.cookie = "".concat(name, "=").concat(encodeURI(value) + expires + path + domain + secure + sameSite); // 转码并赋值
      }
    },
